"""ElGamal signature key-recovery attack via nonce-space birthday collision.

The oracle below samples its per-signature nonce k from [0, log2(p)] instead
of uniformly from Z/(p-1)Z, so a birthday-paradox search over ~sqrt(log2(p))
signatures finds two different messages sharing the same r = g^k. Once such
a collision is found, both k and the private key sk fall out of the ElGamal
signing equation via modular arithmetic (see key_recovery_attack).
"""
...
def oracle(m):
    """Signs message m with the vulnerable nonce-space ElGamal oracle.

    The nonce k is drawn from [0, log2(p)] instead of uniformly from
    Z/(p-1)Z -- this is the vulnerability the attack below exploits. sk is a
    module-local secret, not meant to be read directly by the attack.
    """
